  ...
  <nav ref="menu" class="flex flex-wrap justify-between items-center gap-5">
    <div class="flex items-center gap-5">
      <NuxtLink to="/" @click="closeMenu">
        <NuxtImg class="rounded-full max-h-16 max-w-16 border-vue-purple dark:border-vue-green border-[3px]"
          src="/images/sergio-azocar.jpeg" alt="Sergio Azócar" />
      </NuxtLink>
    ...
    <div id="navbar-default" class="w-full sm:block sm:w-auto" :class="{ hidden: !showMenu }">
      <ul
        ...
        <li v-for="link in menuLinks" :key="link.path" class="w-full" @click="closeMenu">
          <NuxtLink active-class="!text-vue-purple dark:!text-vue-green" :to="link.path"
